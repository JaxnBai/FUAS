...
# import fit2
# sys.path.insert(0, '/root/autodl-tmp/Medical_SAM2/')
...
class ArgsConfig:
    ...
    def niito2D(self,filepath, outputpath,id):
        ...
        for current_slice in loop:
            ...
            data = image_array[:,:, current_slice]  # 保存该切片，可以选择不同方向。当前保存Sag方向，可自行调整顺序
            data = (data - np.min(data)) / (np.max(data) - np.min(data)) * 255  # 归一化
            data = data.astype(np.uint8)
            # data = cv2.equalizeHist(data)  # 直方图均衡化，增强对比度

            #print(data.shape)
        
            # 切片命名
            image_name = "{}".format(str(current_slice )) + ".png"
            # 保存
            imageio.imwrite(image_name, data)

            # 移动到输出文件夹
            src = image_name
            shutil.move(src, image_dir)
    ...
    def mask_to_nii(self):
        ...
        # 遍历每个子文件夹
        for folder_name in tqdm(sorted(os.listdir(input_root))):
    ...
    def resize_images(self,output_size=(1024, 1024) ):
        ...
        print(f"所有图像已调整为 {output_size} 并保存至 {save_to}")
    ...
    def save(self,collected_info):
        ...
        for patient_id,slice_idx,predicted_dose in collected_info:
            ...
            shutil.copy(os.path.join(args.pred_mask,patient_id,f'{slice_idx}.npy'),os.path.join(dir_path,f'{slice_idx}.npy'))
            ...
            with open(os.path.join(dir_path,f'{slice_idx}.txt'), 'w') as f:
...
class Visualize:
    ...
    def process_folders(self):
        ...
        # breakpoint()
        for sub_npy_folder in tqdm(sub_npy_folders, desc="处理小文件夹进度"):
            ...
            for npy_filename in tqdm(npy_files, desc=f"处理 {sub_npy_folder} 文件夹内文件进度", leave=False):
...
#sk-vurwvorrxtvqscrvtzhvimlridiudnjjkzvmpnzaedmofwrt
        principles = """
        治疗计划制定的基本原则
        一、病史相关
...
class MriStratery:
    ...
    def call_model(self, prompt):
        ...
        headers = {
            "Authorization": "Bearer sk-vurwvorrxtvqscrvtzhvimlridiudnjjkzvmpnzaedmofwrt",  # 请替换成你的真实 key
...
class Planner:
    def __init__(self):
        ...
        self.args.image_newsize_path = os.path.join(os.path.dirname(self.args.image_path), 'image_newsize')
        self.args.visual_path = os.path.join(os.path.dirname(self.args.image_path), 'visual_image')
        self.args.mask_nii_path = os.path.join(os.path.dirname(self.args.image_path), 'mask_nii')
        self.args.image_nii_path = os.path.join(os.path.dirname(self.args.image_path), 'image_nii')
        self.args.pred_mask = os.path.join(os.path.dirname(self.args.image_path), 'pred_mask')
        self.args.final_path = os.path.join(os.path.dirname(self.args.image_path), 'final')
        self.args.mri_input = os.path.join(os.path.dirname(self.args.image_path), 'mri_input')
        self.args.mri_output = os.path.join(os.path.dirname(self.args.image_path), 'mri_output')
    ...
    def parse_args(self): 
        ...
        parser.add_argument('-sam_ckpt', type=str, default='/root/autodl-tmp/Medical_SAM2/checkpoints/sam2_hiera_small.pt' , help='sam checkpoint address')
        ...
        parser.add_argument('-seg_type', type=str, default='nothing input', help='initial mem_layers learning rate')

        parser.add_argument(
        '-niigz_path',
        type=str,
        default='./dataset/20例影像科报告文本',   
        help='The path of segmentation output data')
    
        parser.add_argument(
        '-image_nii_path',
        type=str,
        default='/root/autodl-tmp/agent/image_nii',
        help='The path of segmentation output data')
        
        parser.add_argument(
        '-mask_nii_path',
        type=str,
        default='/root/autodl-tmp/agent/mask_nii',
        help='The path of segmentation output data')

        print("Adding argument -image_path")
        ...
        '-origin_mask',
        type=str,
        default='/root/autodl-tmp/agent/test_mask',
        ...
        '-pred_mask',
        type=str,
        default='/root/autodl-tmp/segdatasets/output/myoma/mask/exp5.3',
        help='The path of segmentation output data')
        ...
        '-dose_csv_path',
        type=str,
        default='/root/autodl-tmp/gbli/ultrasound_dose_prediction/data/total_id_doses1.csv',
        help='The path of segmentation output data')    
...
# python class_agent.py -prompt nothing -image_path ./data/image  -niigz_path ./dataset/20例影像科报告文本 -dose_model ./dose_model/dose_model_BayesSearchCV.joblib  -pretrain ./seg_model/nothing_v1.pth

# python agent.py -prompt click -origin_mask /root/autodl-tmp/agent/test_mask  -image_path  /root/autodl-tmp/agent/image_test  -pretrain /root/autodl-tmp/Medical_SAM2/logs/BTCV_MedSAM2_2025_04_16_12_42_06/Model/latest_epoch_9_tol_0.053031034767627716.pth

# python agent.py -prompt bbox -origin_mask /root/autodl-tmp/click/test_mask  -image_path  /root/autodl-tmp/click/image_test  -pretrain /root/autodl-tmp/Medical_SAM2/logs/BTCV_MedSAM2_2025_04_23_13_23_09/Model/47_tol_0.013138828799128532.pth