...
class SAM2Base(torch.nn.Module):
    ...
    def _apply_non_overlapping_constraints(self, pred_masks):
        ...
        max_obj_inds = torch.argmax(pred_masks.detach(), dim=0, keepdim=True)  # detach to prevent gradient flow through argmax
        # "batch_obj_inds": object index of each object slice (along dim 0) in `pred_masks`
        batch_obj_inds = torch.arange(batch_size, device=device)[:, None, None, None]
        keep = max_obj_inds == batch_obj_inds
        
        # Create a mask of -10.0 values with the same shape as pred_masks
        suppression = torch.full_like(pred_masks, -10.0)
        # Use the mask to blend between pred_masks and suppression values
        pred_masks = pred_masks * keep.float() + suppression * (~keep).float()
